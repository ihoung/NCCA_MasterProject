...
def uninstall_module(location):
    ...
    if Path(location + "modules/EditableShadingTool.mod").is_file():
        os.remove(location + "modules/EditableShadingTool.mod")
    ...
    for folder in folders:
        if folder.startswith("20") and int(folder) < 2022:
            with open(f"{location}/{folder}/Maya.env", "w+") as env_file:
                if "EDITABLE_SHADING_PLUGIN_ROOT" in env_file.read():
                    lines = env_file.readlines()
                    for line in lines:
                        if 'EDITABLE_SHADING_PLUGIN_ROOT' in line:
                            lines.remove(line)
